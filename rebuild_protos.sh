...
curl \
  ...
  https://api.github.com/repos/apache/arrow/contents/cpp/proto/substrait/extension_rels.proto\?ref\=main \
  --output "$proto_dir"/extension_rels.proto

# Modify Substrait namespace references to use substrait.ibis
sed -i 's/substrait\//substrait\/ibis\//g' "$proto_dir"/extension_rels.proto
sed -i 's/\ substrait\./ .substrait./g' "$proto_dir"/extension_rels.proto
sed -i 's/\.substrait\./.substrait.ibis./g' "$proto_dir"/extension_rels.proto
